...
require_once($CFG->dirroot . '/course/format/gridiron/lib.php');

class format_gridiron_renderer extends format_section_renderer_base {
    ...
    protected function page_title() {
        return get_string('sectionname', 'format_gridironiron');
    }
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused) {
        ...
        echo html_writer::start_tag('div', array('id' => 'gridiconcontainer', 'role' => 'navigation',
            'aria-label' => get_string('gridimagecontainer', 'format_gridiron')));
        ...
        echo html_writer::start_tag('div', array('id' => 'gridshadebox_content', 'class' => 'hide_content',
            'role' => 'region',
            'aria-label' => get_string('shadeboxcontent', 'format_gridiron')));

        echo html_writer::tag('img', '', array('id' => 'gridshadebox_close', 'style' => 'display:none;',
            'src' => $this->output->pix_url('close', 'format_gridiron'),
            'role' => 'link',
            'aria-label' => get_string('closeshadebox', 'format_gridiron')));
        echo html_writer::tag('img', '', array('id' => 'gridshadebox_left', 'class' => 'gridshadebox_arrow',
            'style' => 'display:none;',
            'src' => $this->output->pix_url('arrow_l', 'format_gridiron'),
            'role' => 'link',
            'aria-label' => get_string('previoussection', 'format_gridiron')));
        echo html_writer::tag('img', '', array('id' => 'gridshadebox_right', 'class' => 'gridshadebox_arrow',
            'style' => 'display:none;',
            'src' => $this->output->pix_url('arrow_r', 'format_gridiron'),
            'role' => 'link',
            'aria-label' => get_string('nextsection', 'format_gridiron')));
    ...
    protected function section_edit_controls($course, $section, $onsectionpage = false) {
        ...
        if (has_capability('moodle/course:setcurrentsection', $coursecontext)) {
            if ($course->marker == $section->section) {  // Show the "light globe" on/off.
                $url->param('marker', 0);
                $strmarkedthissection = get_string('markedthissection', 'format_gridiron');
                $controls[] = html_writer::link($url,
                                    html_writer::empty_tag('img', array('src' => $this->output->pix_url('i/marked'),
                                        'class' => 'icon ', 'alt' => $strmarkedthissection)),
                                    array('title' => $strmarkedthissection, 'class' => 'editing_highlight'));
            } else {
                $strmarkthissection = get_string('markthissection', 'format_gridiron');
    ...
    private function make_block_topic0($course, $sections, $modinfo, $editing, $urlpicedit, $streditsummary,
        ...
        if ($editing) {
            ...
            if ($this->topic0_at_top) {
                $strhidesummary = get_string('hide_summary', 'format_gridiron');
                $strhidesummaryalt = get_string('hide_summary_alt', 'format_gridiron');
                ...
                echo html_writer::link(
                        $this->courseformat->grid_moodle_url('mod_summary.php', array(
                            ...
                            'src' => $this->output->pix_url('into_grid', 'format_gridiron'),
    ...
    private function make_block_icon_topics($contextid, $modinfo, $course, $editing, $hascapvishidsect,
        ...
        if ($this->settings['newactivity'] == 2) {
            ...
            if (!file_exists("$CFG->dirroot/course/format/gridiron/pix/new_activity_" . $currentlanguage . ".png")) {
                $currentlanguage = 'en';
            }
            $url_pic_new_activity = $this->output->pix_url('new_activity_' . $currentlanguage, 'format_gridiron');
        ...
        if ($editing) {
            $streditimage = get_string('editimage', 'format_gridiron');
            $streditimagealt = get_string('editimage_alt', 'format_gridiron');
        }
            ...
            if ($showsection) {
                ...
                if (isset($sectionimage->image) && ($sectionimage->displayedimageindex < 1)) {
                    ...
                    $sectionimage = $this->courseformat->setup_displayed_image($sectionimage, $contextid,
                        $this->settings);
                    if (format_gridiron::is_developer_debug()) {
                ...
                if ($course->coursedisplay != COURSE_DISPLAY_MULTIPAGE) {
                    ...
                    if (is_object($sectionimage) && ($sectionimage->displayedimageindex > 0)) {
                        ...
                        $imgurl = $this->output->pix_url('info', 'format_gridiron');
                        $showimg = true;
                    }
                    if ($showimg) {
                        echo html_writer::empty_tag('img', array(
                    ...
                    if ($editing) {
                        ...
                        if ($section == 0) {
                            $strdisplaysummary = get_string('display_summary', 'format_gridiron');
                            $strdisplaysummaryalt = get_string('display_summary_alt', 'format_gridiron');
                            ...
                            echo html_writer::empty_tag('br') . html_writer::link(
                                    $this->courseformat->grid_moodle_url('mod_summary.php', array(
                                        ...
                                        'src' => $this->output->pix_url('out_of_grid', 'format_gridiron'),
    ...
    private function make_block_topics($course, $sections, $modinfo, $editing, $hascapvishidsect, $streditsummary,
        ...
        for ($section = 1; $section <= $course->numsections; $section++) {
            ...
            if ($hascapvishidsect || ($thissection->visible && $thissection->available)) {
                ...
                echo html_writer::tag('p', get_string('hidden_topic', 'format_gridiron'));
        ...
        $editing = $PAGE->user_is_editing();
         $links = $DB->get_records('format_grid_link',array('courseid'=>$course->id),'position');
            ...
            echo html_writer::start_tag('div', array('id' => 'morecourse'));
                ...
                if($editing){
                    echo html_writer::start_tag('li');
                        echo html_writer::link($this->courseformat->grid_moodle_url('editlink.php', array('courseid'=>$course->id)),get_string('addlink','format_gridiron'));
        ...
        if(count($menuAct) > 0 ||$editing ){
            ...
            if($editing){ 
                echo html_writer::start_tag('li');
                        ...
                        echo html_writer::link($this->courseformat->grid_moodle_url('editicon.php', array('courseid'=>$course->id)),get_string('addicon','format_gridiron'));
                        echo html_writer::link($this->courseformat->grid_moodle_url('orderimage.php', array('courseid'=>$course->id)),get_string('reorder','format_gridiron'));
                echo html_writer::end_tag('li');
            }  
            echo html_writer::end_tag('ul');
            echo html_writer::end_tag('div');
